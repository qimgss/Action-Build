...
#ifdef CONFIG_KSU_SUSFS
    // 当启用 SUSFS 时，ksu_sulog_capture 的第三个参数为指针
    pending = ksu_sulog_capture(KSU_SULOG_EVENT_IOCTL_GRANT_ROOT, NULL, user_arg_null_ptr(), gfp);
#else
    // 未启用 SUSFS 时，构造一个空的 struct user_arg_ptr 值传入
    struct user_arg_ptr null_argv = { .ptr.native = NULL, .is_compat = 0 };
    pending = ksu_sulog_capture(KSU_SULOG_EVENT_IOCTL_GRANT_ROOT, NULL, null_argv, gfp);
#endif
